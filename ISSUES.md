...
**Fixed (2026-06-21, PR pending):** `CorruptSuperblock { defects: Vec<SlotDefect> }` where `SlotDefect { slot: u32, defect: SuperblockDefect }` and `SuperblockDefect = BadChecksum | BadMagic | BadCount(u32)` — both re-exported from the crate root. The three torn-slot rules were factored out of `deserialize` into a shared `validate(buf) -> Result<(), SuperblockDefect>` (checksum-first order preserved), so the HOT path is untouched (`deserialize` keeps its `Option` signature, `validate(buf).ok()?`) and the cause detail is built ONLY on failure: `select() == None` now maps via `ok_or_else(|| CorruptSuperblock { defects: Superblock::diagnose(&candidates) })`, where `diagnose` re-runs `validate` per candidate (cold path). `Display` summarizes per slot ("no valid superblock found: slot 0: bad checksum; slot 1: bad magic"). Python stays **message-only** (the `CorruptSuperblockError` type is unchanged; the enriched `Display` flows through `msg`) — no structured `.defects` accessor (YAGNI for a drop-and-reopen error). The I115 `corrupt_magic_surfaces_*` test was upgraded from a variant-level pin to a **cause-level** pin (asserts slots 0 and 1 report `BadMagic`). `page_size`/`format_version` mismatches stay their own caller-raised errors (out of scope). Stylistic note: `SuperblockDefect` carries `#[allow(clippy::enum_variant_names)]` for the shared `Bad*` prefix.
...
#### I133. `ARCHITECTURE.md` "common 16-byte header" vs `COMMON_HEADER_SIZE = 12` [deepdive 2026-06-21] — **P3** ✅ FIXED 2026-06-21 (briefly deferred; a layout workflow proved there's no collision — the constant was just stale)
