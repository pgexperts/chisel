...
**Chosen.** A sidecar file `<db_path>.spillway` absorbs LRU-tail dirty pages when the cache is full of dirty (un-evictable) pages, turning the cache into a strict bound rather than an elastic one. `Options::spillway_max_bytes` (default `1024 × cache_max_bytes`) bounds how much may be spilled at once, not how large the sidecar file gets — slot indices come from a cursor that only rewinds on truncate, so re-spilling a rehydrated page costs a new slot. That asymmetry is deliberate: reclaiming slots mid-transaction would need a free list and a re-entrancy story for the drain, and the file is wholly discardable at commit anyway. That discardability is the shared root of both this choice and the decision never to fsync the spillway — the two are siblings, not cause and effect. Each slot carries its own XXH3 over `page_id ‖ page_bytes`; it is *never* fsynced and is truncated at open and at every commit/rollback. Setting `spillway_max_bytes = 0` disables it and restores `CacheFull`-at-cap.
