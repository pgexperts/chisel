...
- **Durability over performance.** Every commit performs three `fsync` calls: the I28 pre-drain flush, the data-page flush, then the superblock. The pre-drain is unconditional (`commit.rs` calls `cache.flush()` before `persist_freemap`, and `PageCache::flush` fsyncs even with zero dirty pages), so three is the floor, not the loaded case — budget IOPS accordingly. Every page carries an XXH3 checksum validated on load. Any `fsync` failure poisons the manager (fsyncgate makes retry unsafe).
...
| 5 | `handle_table.rs` | Radix tree mapping `u64` handle → `(page_id, slot_index)`. Implements its own copy-on-write through an `alloc` closure the transaction layer injects (`cow_alloc`: freemap reuse first, file extension as fallback); `create_root` is the module's only direct `PageCache::new_page` call. | Capacity = `510 × 1021^depth`; `find_leaf` short-circuits on `handle ≥ capacity` to `Ok(None)` (I26) — except at depth 6, where capacity saturates to `u64::MAX` and every handle is genuinely in reach. `insert`'s growth loop is bounded by the same `MAX_DEPTH`. |
...
The page cache enforces a strict cap (`Options::cache_max_bytes`). When the cache is full and every entry is dirty (so nothing is evictable), overflow dirty pages spill to a sidecar file `<db_path>.spillway` rather than returning `CacheFull`. `Options::spillway_max_bytes` (default `1024 × cache_max_bytes` = 8 GiB at the 8 MiB cache default) bounds the spillway's LIVE RESIDENT SET, not the sidecar file's length: `Spillway::spill` charges the cap against `slots.len() × payload_size`, while slots are addressed by the monotonic `next_slot_index` cursor that `forget` never rewinds. Rehydrating a page calls `forget`, so a spill → rehydrate → re-spill cycle consumes a new slot per lap; only `truncate` (commit/rollback) reclaims the tail. A transaction whose working set exceeds `max_pages` can therefore grow `<db>.spillway` well past this cap with no `SpillwayFull` — size volumes for spill churn, not for `spillway_max_bytes`. `SpillwayFull { limit_bytes }` fires when the cache and the LIVE spillway set are both exhausted. Setting `spillway_max_bytes = 0` disables the spillway and restores `CacheFull`-at-cap semantics.
...
**Transaction-scoped recycle pool (`TxnPageRecycle`).** The session-COW dedup above is the freemap's answer to within-transaction churn, and it is safe there only because nothing mutates the freemap tree inside a savepoint scope. The handle-table and membership radices get no such escape — mutating inside savepoints is the point of savepoints — so they use a different mechanism for the same problem: `cow_alloc` draws COW targets from a pool of pages this transaction **both allocated and superseded**, consulted before the committed bitmap. Nothing is ever mutated in place.

Both halves of that membership rule are load-bearing, and each retires one class of restorable snapshot. *Allocated by this transaction* retires the committed superblock's roots: `allocate_first` yields only ids the committed bitmap marks free, and `new_page` is monotonic above the begin watermark. *Superseded by this transaction* retires `current_roots`, because the pool is fed only at INSTALL sites, after the replacement root is written. Savepoint roots are retired by gating feed and draw on `savepoints.is_empty()` and draining the pool on savepoint push, so the pool is provably empty and inert for the whole scope.

The in-place alternative — giving both radices the freemap's `session_owned` treatment — corrupts data, and the reasoning is recorded in ADR 0029's neighbour, ADR 0026. In short: all three mutation paths discard a candidate radix root by restoring a saved root *id*, which under in-place mutation is unchanged, so the discard silently becomes a no-op over an already-live mutation. Two tripwire tests encode that hazard and fail under it.
